...
## 创建系统初始必要目录
```
mkdir -p /yatai-java/data
mkdir -p /yatai-java/script
mkdir -p /yatai-java/config
mkdir -p /yatai-java/yt_logs
```
## 安装mysqld服务
```
unzip bushu.zip
...
数据库初始化
mysql -uroot -p'abc!@#123' -e"CREATE SCHEMA feg2022_school DEFAULT CHARACTER SET utf8mb4";
mysql -uroot -p
use feg2022_school
source x.sql
-----------------------------------------
...
添加数据库访问账户
CREATE USER `feg_app`@`%` IDENTIFIED WITH mysql_native_password BY 'feg_app2020@_学校id';
GRANT Alter, Alter Routine, Create, Create Routine, Create Temporary Tables, Create View, Delete, Drop, Event, Execute, Index, Insert, Lock Tables, References, Select, Show View, Trigger, Update ON `feg2022\_school`.* TO `feg_app`@`%`;
FLUSH PRIVILEGES;
-----------------------------------------
如果mysql安装完毕之后，使用工具（navicat）连不上数据库，记得检查防火墙是否默认打开处于运行状态
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ5NzQ5NTcwMF19
-->